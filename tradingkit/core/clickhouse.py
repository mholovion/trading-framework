...
def _validate_identifier(name: Any, *, kind: str = "identifier") -> str:
    """
    Guard against SQL injection through table/column names that get interpolated as
    raw (unquoted) SQL syntax — e.g. f"CREATE TABLE {table_name} (...)". Unlike
    query *values* (escaped via _fmt/_interpolate below), identifiers can't be safely
    quoted away, so any name that isn't a plain [A-Za-z_][A-Za-z0-9_]* token is rejected
    outright. table_name/column names here can originate from a user-authored script
    (AggregationScript.OUTPUT_TABLE/OUTPUT_SCHEMA, ConnectionScriptSource.TABLE_NAME),
    so this boundary is not just defensive — it's reachable from that surface.
    """
...
def _validates_identifiers(*param_names: str):
    """
    Method decorator: validate the named parameters as ClickHouse identifiers (see
    _validate_identifier) before the method body runs, instead of repeating the same
    inline check in every method that takes a table/raw_table name. Methods whose
    identifier list isn't a fixed parameter (e.g. per-Fold aliases derived from a
    `folds` list) still validate those inline — this only covers plain named params.
    """
    def _decorator(fn):
